...
namespace Angle {
	...
	//	inline Digree operator-() const { return -angle_; }
	//	/// @brief 明示的にDigreeを<Radian>に変換できる
	//	inline explicit operator Angle::Radian() const noexcept;
	...
	//	//Radian(Digree value) { angle_ = value.Get() * Dig2Rad; };
	//	Radian(const Radian &) = default;
	//	inline Radian &operator=(const Radian &digree) {
	//		angle_ = digree.angle_;
	//		return *this;
	//	}
